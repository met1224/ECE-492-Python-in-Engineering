...
def calLetterGrade(points, gradescale=None):
    ...
    for i in range(len(gradescale) - 1):
        if points >= gradescale[i]:
            return _get_grade_letter(i, len(gradescale) - 2)

    return 'F'


def _get_grade_letter(index, max_index):
    """
    Helper function to get the letter grade based on the index and the maximum index.
